...
import { X, Plus, Trash2, Save, User, Home, Calendar, Calculator, Percent, CreditCard } from 'lucide-react';
...
export const InvoiceForm: React.FC<InvoiceFormProps> = ({ onSave, onCancel }) => {
  const [formData, setFormData] = useState({
    ...
    installments: [{ dueDate: '', percentage: 100, amount: 0 }],
  });
  ...
  const handleAddInstallment = () => {
    setFormData({
      ...formData,
      installments: [...formData.installments, { dueDate: '', percentage: 0, amount: 0 }],
    });
  };
  ...
  const handleInstallmentChange = (index: number, field: string, value: any) => {
    ...
    if (field === 'percentage') {
      const totalWithVat = calculateGrandTotal();
      newInstallments[index].amount = (totalWithVat * parseFloat(value)) / 100;
    }
  ...
  useEffect(() => {
    if (formData.enableInstallments) {
      ...
      const updatedInstallments = formData.installments.map(inst => ({
        ...inst,
        amount: (total * parseFloat(inst.percentage.toString() || '0')) / 100
      }));
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-lg">
      {/* Header */}
      <div className="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-amber-50 to-amber-100">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-900">สร้างใบแจ้งหนี้</h2>
            <p className="text-sm text-gray-600 mt-1">กรอกข้อมูลเพื่อสร้างใบแจ้งหนี้ใหม่</p>
          </div>
          <button
            onClick={onCancel}
      ...
      <form onSubmit={handleSubmit}>
        <div className="p-6 space-y-6 max-h-[calc(100vh-16rem)] overflow-y-auto">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                  />
                </div>
              </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-600 rounded-lg text-sm font-medium transition"
              >
                <Plus size={16} />
                เพิ่มรายการ
              </button>
                ...
                >
                  <div className="col-span-12 md:col-span-5">
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                    />
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-1">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                />
              </div>
              <div className="md:col-span-1">
                <label className="block text-sm font-medium text-gray-700 mb-2">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">แบ่งชำระเป็นงวด</p>
                </div>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, enableInstallments: e.target.checked })}
                  className="sr-only peer"
                />
              ...
              <div className="space-y-3 pt-4 border-t border-gray-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-gray-600">
                    รวม: {getTotalInstallmentPercentage().toFixed(2)}% / 100%
                  </span>
                  <button
                    type="button"
                    onClick={handleAddInstallment}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 rounded-lg text-xs font-medium transition"
                  >
                    <Plus size={14} />
                    เพิ่มงวด
                  </button>
                ...
                {formData.installments.map((installment, index) => (
                  <div key={index} className="grid grid-cols-12 gap-3 p-3 bg-emerald-50 rounded-lg border border-emerald-200">
                    <div className="col-span-1 flex items-center justify-center">
                    ...
                    <div className="col-span-3 md:col-span-2">
                      <input
                        ...
                        onChange={(e) => handleInstallmentChange(index, 'percentage', parseFloat(e.target.value) || 0)}
                        placeholder="%"
                        className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm"
                      />
                    </div>
                    <div className="col-span-2 md:col-span-4 flex items-center">
                      <span className="text-sm font-semibold text-gray-900">
                        ฿{installment.amount.toLocaleString('th-TH', { minimumFractionDigits: 2 })}
                      </span>
                    </div>
                    <div className="col-span-1 flex items-center justify-center">
                ...
                {Math.abs(getTotalInstallmentPercentage() - 100) > 0.01 && (
                  <div className="text-xs text-amber-600 bg-amber-50 p-3 rounded-lg">
                    ⚠️ กรุณาตรวจสอบให้ผลรวมของเปอร์เซ็นต์เท่ากับ 100%
                  </div>
                )}
              </div>
            )}
          ...
          {/* Summary */}
          <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-6 space-y-3 border-2 border-amber-200">
            ...
            <div className="space-y-2">
              ...
              <div className="pt-3 border-t-2 border-amber-300">
                <div className="flex items-center justify-between">
                  <span className="text-base md:text-lg font-bold text-gray-900">ยอดรวมทั้งสิ้น</span>
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-end gap-3 sticky bottom-0">
          ...
          <button
            type="submit"
            className="flex items-center gap-2 px-6 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-medium transition"
          >
            <Save size={18} />
            บันทึกใบแจ้งหนี้
          </button>
